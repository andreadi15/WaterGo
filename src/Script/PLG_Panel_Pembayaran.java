...
//  Fungsi Pembuatan ID Order Baru Otomatis  
    private String create_id_detail(){
        String lastId = null;
        String sql = "SELECT id_detail FROM detail_pesanan ORDER BY id_detail DESC LIMIT 1"; // ambil ID terakhir
        try (PreparedStatement ps = AP_Database.con.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {

            if (rs.next()) {
                lastId = rs.getString("id_detail"); // misal "DTL005"
            }
        } 
        catch (Exception e) 
        {
        ...
        String newId = "DTL001"; // default kalau tabel kosong

        if (lastId != null) {
            int number = Integer.parseInt(lastId.substring(3)); // ambil angka dari "DTL005" → 5
            number++; // tambah 1
            newId = String.format("DTL%03d", number); // hasil: "DTL006"
        }
        return newId;
    }
...
//  Fungsi Checkout Tombol Konfirmasi Pesanan
    private void checkout(){
        String id_order = create_id_order();
        for (String idProduk : cart.keySet()) {
            Map<String, Object> data = cart.get(idProduk);
            ...
            String sql="INSERT INTO detail_pesanan(id_detail, id_pesanan, id_product, jumlah, harga_satuan, subtotal) VALUES(?, ?, ?, ?, ?, ?)";
            try(PreparedStatement ps = AP_Database.con.prepareStatement(sql))
            {           
                ps.setString(1, create_id_detail());
